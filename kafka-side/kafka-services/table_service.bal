    ...
    resource function onMessage(kafka:SimpleConsumer simpleConsumer, kafka:ConsumerRecord[] records) returns error? {
        foreach var entry in records {
            ...
            match (entry.topic) {
                ...
                "leave-table" => {
                    check removeGuestsFromTable(msg);
                }
                "request-bill" => {
...
function removeGuestsFromTable(string msg) returns error? {
    io:StringReader sr = new (msg, encoding = "UTF-8");
    json j = check sr.readJson();
    io:println(tables);
    tables[j["table_name"].toString()] = "";
    var x = itemOrdersForGuests.remove(j["booking_id"].toString());
    var y = ordersForGuests.remove(j["booking_id"].toString());
    io:println(tables);
}
...
function doPayment(string msg) returns error? {
    io:StringReader sr = new (msg, encoding = "UTF-8");
    json j = check sr.readJson();
    float amt = check float.convert(j["amount_paid"].toString());
    if (amt >= getCost(j["booking_id"].toString())) {
        // reset ordered items and the total cost
        itemOrdersForGuests[j["booking_id"].toString()] = "";
        ordersForGuests[j["booking_id"].toString()] = 0;
    }
}
